...
public class AssetResidualConfigServiceImpl implements AssetResidualConfigService {
    ...
    public void save(AssetResidualConfigSaveReqVO reqVO){
        ...
        if(old!=null){ monthMapper.logicDelByConfigId(old.getId(),u.getPartnerId(),u.getId(),now); yearMapper.logicDelByConfigId(old.getId(),u.getPartnerId(),u.getId(),now); configMapper.physicalDelDeletedBySkuId(reqVO.getStandardProductSkuId(),u.getPartnerId()); configMapper.logicDelBySkuId(reqVO.getStandardProductSkuId(),u.getPartnerId(),u.getId(),now);}        
