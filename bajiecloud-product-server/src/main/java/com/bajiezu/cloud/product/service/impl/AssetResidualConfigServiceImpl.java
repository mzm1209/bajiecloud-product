...
public class AssetResidualConfigServiceImpl implements AssetResidualConfigService {
    ...
    public AssetResidualConfigDetailRespVO detail(AssetResidualConfigQueryReqVO reqVO){
        ...
        resp.setOfficialPrice(toDisplayAmount(c.getOfficialPrice())); resp.setDepreciationRuleType(c.getDepreciationRuleType()); resp.setDepreciationRuleSubType(c.getDepreciationRuleSubType()); resp.setRemark(c.getRemark()); resp.setStatus(c.getStatus());
    ...
    public void save(AssetResidualConfigSaveReqVO reqVO){
        ...
        BigDecimal lastYearEnd=officialPriceAmount;
        List<AssetResidualMonthConfig> ml=new ArrayList<>();
        for(int y=1;y<=3;y++){
            BigDecimal accum=BigDecimal.ZERO; BigDecimal yearBegin=(y==1)?officialPriceAmount:lastYearEnd; BigDecimal yearDep=BigDecimal.ZERO;
            for(int m=1;m<=12;m++){
                ...
                AssetResidualMonthConfig mo=new AssetResidualMonthConfig(); mo.setAssetResidualConfigId(c.getId()); mo.setAssetResidualYearConfigId(null); mo.setPartnerId(u.getPartnerId()); mo.setUseYear(y); mo.setUseMonth(m); mo.setGlobalMonth(g); mo.setDepreciationRuleValue(in.getDepreciationRuleValue()); mo.setBeginValue(toAmountLong(begin)); mo.setDepreciationAmount(toAmountLong(dep)); mo.setResidualValue(toAmountLong(residual)); mo.setAccumulatedDepreciationAmount(toAmountLong(accum)); mo.setCurrentPurchaseAmount(toAmountLong(residual)); mo.setCreateBy(u.getId()); mo.setUpdateBy(u.getId()); mo.setCreateTime(now); mo.setUpdateTime(now); mo.setIsDeleted(0); ml.add(mo); yearDep=yearDep.add(dep);
            }
            AssetResidualYearConfigVO yin=years.get(y); BigDecimal yearEndResidual=toAmountDecimal(ml.get(ml.size()-1).getResidualValue()); AssetResidualYearConfig yo=new AssetResidualYearConfig(); yo.setAssetResidualConfigId(c.getId()); yo.setPartnerId(u.getPartnerId()); yo.setUseYear(y); yo.setTotalPriceUpperCoefficient(yin.getTotalPriceUpperCoefficient()); yo.setTotalPriceLowerCoefficient(yin.getTotalPriceLowerCoefficient()); yo.setYearBeginValue(toAmountLong(yearBegin)); yo.setYearDepreciationAmount(toAmountLong(yearDep)); yo.setYearEndResidualValue(toAmountLong(yearEndResidual)); yo.setTotalPriceUpperLimit(toAmountLong(yearBegin.multiply(yin.getTotalPriceUpperCoefficient()))); yo.setTotalPriceLowerLimit(toAmountLong(yearBegin.multiply(yin.getTotalPriceLowerCoefficient()))); yo.setCreateBy(u.getId()); yo.setUpdateBy(u.getId()); yo.setCreateTime(now); yo.setUpdateTime(now); yo.setIsDeleted(0); yearMapper.insert(yo); lastYearEnd=yearEndResidual;
    ...
    private BigDecimal toYuanDecimal(BigDecimal v){ return v==null?null:v.setScale(4,RoundingMode.HALF_UP); }
    ...
    private BigDecimal toDisplayAmount(Long storedValue){ return storedValue==null?null:toAmountDecimal(storedValue).setScale(4,RoundingMode.HALF_UP); }
    ...
    private void validate(AssetResidualConfigSaveReqVO r){
        ...
        if (r.getOfficialPrice() == null || r.getOfficialPrice().compareTo(BigDecimal.ZERO) <= 0) throw exception(ASSET_RESIDUAL_PARAM_INVALID);
    ...
    private AssetResidualYearConfigVO toYearVO(AssetResidualYearConfig y){ AssetResidualYearConfigVO v=new AssetResidualYearConfigVO(); v.setUseYear(y.getUseYear()); v.setTotalPriceUpperCoefficient(y.getTotalPriceUpperCoefficient()); v.setTotalPriceLowerCoefficient(y.getTotalPriceLowerCoefficient()); v.setYearBeginValue(toDisplayAmount(y.getYearBeginValue())); v.setYearDepreciationAmount(toDisplayAmount(y.getYearDepreciationAmount())); v.setYearEndResidualValue(toDisplayAmount(y.getYearEndResidualValue())); v.setTotalPriceUpperLimit(toDisplayAmount(y.getTotalPriceUpperLimit())); v.setTotalPriceLowerLimit(toDisplayAmount(y.getTotalPriceLowerLimit())); return v; }
    private AssetResidualMonthConfigVO toMonthVO(AssetResidualMonthConfig m){ AssetResidualMonthConfigVO v=new AssetResidualMonthConfigVO(); v.setUseYear(m.getUseYear()); v.setUseMonth(m.getUseMonth()); v.setGlobalMonth(m.getGlobalMonth()); v.setDepreciationRuleValue(m.getDepreciationRuleValue()); v.setBeginValue(toDisplayAmount(m.getBeginValue())); v.setDepreciationAmount(toDisplayAmount(m.getDepreciationAmount())); v.setResidualValue(toDisplayAmount(m.getResidualValue())); v.setAccumulatedDepreciationAmount(toDisplayAmount(m.getAccumulatedDepreciationAmount())); v.setCurrentPurchaseAmount(toDisplayAmount(m.getCurrentPurchaseAmount())); return v; }
}
